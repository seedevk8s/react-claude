...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      {/* 헤더 */}
      <header className="bg-slate-800 text-white px-8 py-5">
        <h1 className="text-2xl font-bold">📚 BookStore</h1>
        {/* ✏️ JSX 규칙 ③: 빈 태그도 반드시 / 로 닫는다 → <br /> */}
        <p className="text-slate-300 text-sm mt-1">개발자를 위한 기술 도서 쇼핑몰</p>
      </header>

      {/* 메인 */}
      <main className="flex-1 max-w-4xl w-full mx-auto px-6 py-10">

        <h2 className="text-lg font-semibold text-slate-600 mb-6">신간 도서 목록</h2>
        ...
        */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {books.map(book => (
            <div key={book.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex flex-col justify-between">
              ...
              <div className="mt-4">
                ...
                */}
                {book.inStock ? (
                  ...
                  <button className="w-full py-2 bg-gray-100 text-gray-400 text-sm rounded-lg cursor-not-allowed" disabled>
                    품절
                  </button>
                )}
              </div>
      ...
      {/* 푸터 */}
      <footer className="text-center py-5 text-sm text-gray-400 border-t border-gray-200">
